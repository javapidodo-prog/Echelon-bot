...
class LauncherApp(ctk.CTk):
    def __init__(self):
        ...
        self.parser_process: subprocess.Popen | None = None
        ...
        self.parser_card = ModuleCard(self.main, "Парсер объявлений", self.toggle_parser)
        self.parser_card.pack(fill="x", pady=(0, 16))
    ...
    def start_bot(self):
        ...
        args = [python_exe, "--bot"] if getattr(sys, "frozen", False) else [python_exe, "bot.py", "--mode", "bot"]
        try:
            env = os.environ.copy()
    ...
    def start_parser(self):
        python_exe = self._get_python_exe()
        args = [python_exe, "--parser"] if getattr(sys, "frozen", False) else [python_exe, "bot.py", "--mode", "parser"]
        try:
            env = os.environ.copy()
            env["SCRAPER_MODE"] = self.mode_var.get()
            env["SCRAPER_HEADLESS"] = "True" if self.headless_var.get() == "Без окна" else "False"
            ...
            self.parser_card.set_active(True)
            threading.Thread(
                target=self.read_output, args=(self.parser_process, "Парсер"), daemon=True
            ).start()
            self.log(f"Парсер запущен (режим: {self.mode_var.get().upper()}).")
        except Exception as e:
            self.log(f"Ошибка: {e}")
    ...
    def stop_parser(self):
        if self.parser_process:
            ...
            self.parser_card.set_active(False)
            self.log("Парсер остановлен.")
    ...
    def change_mode(self, new_mode: str):
        ...
        if self.bot_process is not None:
            self.log("⚠️ Чтобы бот начал работать в новом режиме, выключите его и включите заново.")
        if self.parser_process is not None:
            self.log("⚠️ Чтобы парсер начал работать в новом режиме, выключите его и включите заново.")
    ...
    def change_headless(self, new_val: str):
        ...
        if self.parser_process is not None:
            self.log("⚠️ Чтобы парсер применил настройки браузера, выключите его и включите заново.")
        if self.dashboard_process is not None:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == "--bot":
            ...
            try:
                asyncio.run(bot.main(run_mode="bot"))
            ...
            try:
                asyncio.run(bot.main(run_mode="parser"))
            except (KeyboardInterrupt, SystemExit):
                pass
