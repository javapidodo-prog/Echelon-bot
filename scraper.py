...
class AvitoScraper:
    ...
    def _clean_price(self, price_str: str) -> int | None:
    ...
    async def _check_for_blocks(self, page: Page):
        ...
        if (
            "докажите, что вы человек" in title_lower
            or "captcha" in title_lower
            or "ой!" in title_lower
        ):
    ...
    async def parse_page(self, page: Page, url: str, city: str) -> list[dict]:
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    "Не удалось найти объявления на странице. "
                    "Возможно, сменилась верстка или Авито заблокировал доступ."
                )
                ...
                screenshot_path = os.path.join(debug_dir, f"error_{time_str}.png")
            ...
            for item in items:
                try:
                    ...
                    if not link_elem:
                        link_elem = await item.query_selector('a')  # Резерв
                    ...
                    if not title_elem:
                        title_elem = await item.query_selector(
                            'h3[itemprop="name"], h2[itemprop="name"], h3, h2'
                        )
                    ...
                    if snippet_elem:
                        ...
                        snippet_elem = await item.query_selector(
                            '[class*="description"], [class*="body-text"]'
                        )
