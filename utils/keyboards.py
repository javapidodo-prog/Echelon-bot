...
def get_main_inline_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📋 Мои отслеживания", callback_data="menu_status")],
            [
                InlineKeyboardButton(text="🏢 Города", callback_data="menu_cities"),
...
def get_settings_keyboard(min_discount: int, dnd_enabled: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📥 Готово", callback_data="menu_status")]
        ]
    )
...
def get_models_keyboard(
    ...
    for model in all_models:
        ...
        price_min_k = price_min // 1000
        label = f"{check}{name} (от {price_min_k}к ₽)"
        rows.append(
            [
                InlineKeyboardButton(
                    text=label,
