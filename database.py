...
async def init_db() -> None:
    async with db_session() as db:
        ...
        await db.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_listings_model_date 
                ON listings(model_id, parsed_at DESC);
        ...
        for item in items:
            ...
            try:
                cur = await db.execute(
                    ...
                    (listing_id, url, title, price, model_id, city, image_url,
                     description_preview, median_at_time, discount_percent)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (
                        str(item["id"]),
...
async def get_new_listings_for_user(user_id: int) -> list[dict]:
    ...
    sql = f"""
        ...
        WHERE um.user_id = ? AND l.parsed_at <= {time_filter}
          ...
          AND NOT EXISTS (
              SELECT 1 FROM sent_listings sl 
              WHERE sl.listing_id = l.listing_id AND sl.user_id = ?
          )
          AND NOT EXISTS (
              SELECT 1 FROM hidden_listings hl 
              WHERE hl.listing_id = l.listing_id AND hl.user_id = ?
          )
    """
...
async def update_model_median(model_id: int, new_median: int) -> None:
    async with db_session() as db:
        await db.execute(
            "UPDATE models SET median_price = ?, last_median_update = CURRENT_TIMESTAMP "
            "WHERE model_id = ?",
            (new_median, model_id),
        )
        await db.commit()
...
async def calculate_median_for_model(model_id: int, days: int = 7) -> int | None:
    async with db_session() as db:
        async with db.execute(
            "SELECT price FROM listings WHERE model_id = ? AND price > 0 "
            "AND parsed_at >= datetime('now', ?) ORDER BY price",
...
async def cleanup_listings(days: int = 30) -> int:
    async with db_session() as db:
        await db.execute(
            "INSERT INTO price_history (model_id, price, city, recorded_at) "
            "SELECT model_id, price, city, parsed_at FROM listings "
...
@db_retry
async def cleanup_old_history_and_logs(
    days_history: int = 180, days_logs: int = 90
) -> tuple[int, int]:
    async with db_session() as db:
        async with db.execute(
        ...
        async with db.execute(
            "DELETE FROM validation_log WHERE notified_at < datetime('now', ?)",
            (f"-{days_logs} days",)
        ) as cur2:
...
async def cleanup_inactive_users(days: int = 180) -> int:
    async with db_session() as db:
        async with db.execute(
            ...
            "(SELECT DISTINCT user_id FROM user_models) AND joined_at < datetime('now', ?)",
            (f"-{days} days",),
        ) as cur:
            await db.commit()
            return cur.rowcount
...
async def get_pending_reports() -> list[dict]:
    async with db_session() as db:
        async with db.execute(
            "SELECT r.*, l.title, l.url FROM reports r LEFT JOIN listings l "
            "ON r.listing_id = l.listing_id WHERE r.status = 'pending' ORDER BY r.created_at DESC"
        ) as cur:
            return [dict(row) for row in await cur.fetchall()]
...
async def log_notification(
    ...
    async with db_session() as db:
        await db.execute(
            ...
            "(user_id, listing_id, model_name, price, median_price, discount_percent) "
            "VALUES (?, ?, ?, ?, ?, ?)",
...
async def update_validation_feedback(
    ...
    async with db_session() as db:
        await db.execute(
            "UPDATE validation_log SET is_false_positive = ?, user_feedback = ? "
...
async def get_validation_stats(days: int = 7) -> dict:
    async with db_session() as db:
        async with db.execute(
            "SELECT COUNT(*) as total, SUM(is_false_positive) as fp, "
            "AVG(discount_percent) as disc FROM validation_log "
        ...
        async with db.execute(
            "SELECT model_name, COUNT(*) as cnt FROM validation_log "
            "WHERE notified_at >= datetime('now', ?) GROUP BY model_name ORDER BY cnt DESC",
            (f"-{days} days",),
        ) as cur:
...
async def get_price_history(model_id: int, days: int = 30) -> list[dict]:
    async with db_session() as db:
        sql = (
            "SELECT price, city, recorded_at FROM price_history "
            "WHERE model_id = ? AND recorded_at >= datetime('now', ?) "
            "UNION ALL "
            "SELECT price, city, parsed_at as recorded_at FROM listings "
            "WHERE model_id = ? AND parsed_at >= datetime('now', ?) "
...
async def get_user_savings(user_id: int) -> int:
    async with db_session() as db:
        async with db.execute(
            "SELECT SUM(median_price - price) FROM validation_log "
            "WHERE user_id = ? AND is_false_positive = 0",
            (user_id,),
        ) as cur:
            row = await cur.fetchone()
