...
async def cb_menu_status(callback: CallbackQuery) -> None:
    ...
    from utils.keyboards import get_main_inline_keyboard
    try:
        await callback.message.edit_text(
            await get_status_text(callback.from_user.id), 
...
@router.callback_query(F.data == "menu_cities")
@error_handler
async def cb_menu_cities(callback: CallbackQuery) -> None:
...
@error_handler
async def cb_menu_models(callback: CallbackQuery) -> None:
...
async def cb_menu_favorites(callback: CallbackQuery) -> None:
    from handlers.commands import cmd_favorites
    await callback.answer()
...
async def cb_menu_settings(callback: CallbackQuery) -> None:
    ...
    await callback.message.edit_text(
        "⚙️ <b>Персональные настройки</b>\n\nЗдесь вы можете настроить порог выгоды для уведомлений и включить тихие часы.",
        reply_markup=kb,
    )
...
async def cb_finish_models(callback: CallbackQuery) -> None:
    ...
    await callback.answer("✅ Модели сохранены. Парсер запущен.")
...
async def _show_listing(
    ...
    city = html.escape(listing.get('city', 'Россия'))
    
    text = f"📱 <b>{title}</b>\n\n"
    text += f"💵 <b>{price_str}</b>\n"
    if median > 0:
        text += f"{discount_emoji} Выгода: <b>{discount}%</b> <i>(в среднем {median}₽)</i>\n"
    text += f"📍 {city}\n"

    nav = []
    if index > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=ListingNavCB(action="prev", model_name=model_name, index=index).pack()))
    else:
        nav.append(InlineKeyboardButton(text="⏹️", callback_data="ignore"))
    
    nav.append(InlineKeyboardButton(text=f"{index + 1} / {len(listings)}", callback_data="ignore"))

    if index < len(listings) - 1:
        nav.append(InlineKeyboardButton(text="➡️", callback_data=ListingNavCB(action="next", model_name=model_name, index=index).pack()))
    else:
        nav.append(InlineKeyboardButton(text="⏹️", callback_data="ignore"))
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="🔗 Авито", url=listing["url"]),
                InlineKeyboardButton(text="⭐ Избранное", callback_data=FavoriteCB(action="add", listing_id=listing["listing_id"]).pack()),
            ],
            [
                InlineKeyboardButton(text="📉 График", callback_data=ListingNavCB(action="history", model_name=model_name, index=index).pack()),
                InlineKeyboardButton(text="❌ Закрыть", callback_data=ListingNavCB(action="exit", model_name=model_name, index=index).pack()),
            ],
            nav
        ]
    )
