...
def _render_listing_rows(rows: list[dict[str, Any]]) -> str:
    ...
    for item in rows:
        ...
        img_url = (
            _escape(raw_img)
            if (
                raw_img.startswith("http://")
                or raw_img.startswith("https://")
                or raw_img.startswith("//")
            )
            else ""
        )
        ...
        title_with_link = (
            ...
            f'style="font-size: 16px; text-decoration: none; color: #48bb78;">'
    ...
    def do_GET(self) -> None:
        ...
        if user and password:
            ...
            try:
                ...
                import secrets
                if not (
                    secrets.compare_digest(req_user, user)
