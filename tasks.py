...
async def cleanup_loop() -> NoReturn:
    while True:
        try:
            ...
            del_hist, del_logs = await db.cleanup_old_history_and_logs(
                days_history=180, days_logs=90
            )
            logger.info(
                f"Cleanup: {del_list} listings, {del_users} users, "
                f"{del_hist} history rows, {del_logs} log rows removed"
            )
                    ...
                    if admin_id:
                        try:
                            await bot.send_message(
                                ...
                                "Пожалуйста, запустите утилиту <b>Captcha Bypass</b> "
                                "в панели управления Echelon (Control Panel), "
        ...
        try:
            ...
            for user in users:
                ...
                if listings := await db.get_new_listings_for_user(user.user_id):
                    ...
                    await db.mark_listings_sent(user.user_id, ids)
            await asyncio.sleep(30)
        except Exception as e:
